...
void viterbi_pi(int *vit_pi, int *real_pi, int *x, double **transition_matrix, double **emission_matrix, double *start_distr, int M, int K, int L)
{
	...
	for (int j = 0; j < M; j++)
	{
		v[0][j] = emission_matrix[j][x[0]]*start_distr[j];
	...
	for (int i = 1; i < L; i++)
	{
		for (int l = 0; l < M; l++)
		{
			v[i][l] = v[i-1][0]*transition_matrix[0][l]*emission_matrix[l][x[i]];
			ptr[i][l] = 0;
			for (int k = 0; k < M; k++)
			{
				if (v[i][l] < v[i-1][k]*transition_matrix[k][l]*emission_matrix[l][x[i]])
				{
					v[i][l] = v[i - 1][k]*transition_matrix[k][l]*emission_matrix[l][x[i]];
					ptr[i][l] = k;
				}
			}
...
void for_back_ward_algorithm(double **prob, int *real_pi, int *x, double **transition_matrix, double **emission_matrix, double *start_distr, int M, int K, int L)
{
	...
	for (int j = 0; j < M; j++)
	{
		f[0][j] = emission_matrix[j][x[0]] * start_distr[j];
	...
	for (int i = 1; i < L; i++)
	{
		...
		{
			for (int k = 0; k < M; k++)
				f[i][l] += f[i-1][k] * transition_matrix[k][l] * emission_matrix[l][x[i]];
		}
	}
	...
	for (int i = L - 2; i > -1; i--)
	{
		...
		{
			for (int l = 0; l < M; l++)
				b[i][k] += b[i+1][l]*transition_matrix[k][l]*emission_matrix[l][x[i + 1]];
		}
	}
	...
	for (int l = 0; l < M; l++)
		backward_prob += b[0][l] * start_distr[l] * emission_matrix[l][x[0]]; /*b[l][0] * transition_matrix[0][l] * emission_matrix[l][x[0]];*/
	...
	for (int i = 0; i < L; i++)
	{
		...
		for (int k = 0; k < M; k++)
		{
			prob[i][k] = f[i][k] * b[i][k] / forward_prob;
			sum += prob[i][k];
		}
		/*if (sum > 1.)
			cout << sum << endl;*/
